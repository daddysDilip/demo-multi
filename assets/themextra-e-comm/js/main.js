...
function getDelete(id) {
  $.get(mainurl+'/cartdelete/'+id, function(response){
    ...
    var str = '';
    var str1 = '';
    var rowCount = $('.detail_box .row').length;

    $("#goCart").html('');
    if(rowCount == 0)
    {
      $('#emptycart').html(EmptyCartlang);
      $('.detail_box').removeClass('col-md-9');
      $('.detail_box').addClass('col-md-12');
      $('.detail_box').html('<h5 style="margin-left: 20px;">'+EmptyCartlang+'</h5><div class="add-cart shopping-btn f-12"><a href="'+mainurl+'" class="text-white">'+ContinueShopping+'</a></div>');
