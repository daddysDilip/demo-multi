...
class FrontEndController extends Controller
{
    ...
    public function add_store(Request $request)
    {
	   ...
	   $tempCompany = $request->all();
	    $CompanyTransection = CompanyTransection::findOrFail($tempCompany['transection_id']);
        $tempCompanyId = $CompanyTransection->CompanyId;
		
	    $tempCompanydata = TempCompany::findOrFail($tempCompanyId);
       
	    $companydata=array(
		 'comapany_name' => $tempCompanydata->comapany_name,
		 'username' => $tempCompanydata->username,
		 'company_logo' => $tempCompanydata->company_logo,
		 'company_email' => $tempCompanydata->company_email,
		 'company_phone' => $tempCompanydata->company_phone,
		 'storeurl' => $tempCompanydata->storeurl,
		 'addressline1' => $tempCompanydata->addressline1,
		 'addressline2' => $tempCompanydata->addressline2,
		 'planid' => $tempCompanydata->planid,
		 'country' => $tempCompanydata->country,
		 'state' => $tempCompanydata->state,
		 'city' => $tempCompanydata->city,
		 'pincode' => $tempCompanydata->pincode,
		 'status' => $tempCompanydata->status,
		 'language' => $tempCompanydata->language,
		 'default_language' => $tempCompanydata->default_language,
		 'password' => $tempCompanydata->password,
		 'plantype' => $tempCompanydata->plantype,
		 'total_amount' => $tempCompanydata->total_amount,
		); 
		
	    $company->fill( $companydata );

	//	echo '<pre>'; print_r($tempCompanydata); echo '</pre>';
		//exit;
		$path = $_SERVER['HTTP_HOST'];
		$password = $companydata['password'];
		$storename = $companydata['username'];
		$url = $companydata['storeurl'];
		...
		*/
		...
		$plans = Plans::findOrFail($companydata['planid']);
		...
		$roles->save();
	  	...
	  	$user = new User();
	  	$user['name'] =$companydata['comapany_name']; 
	  	$user['username'] =$companydata['username']; 
	  	$user['email'] =$companydata['company_email']; 
	  	$user['phone'] =$companydata['company_phone']; 
	  	$user['role'] =$roleid; 
	  	$user['status'] =1; 
		...
		$settings['title'] = $companydata['comapany_name'];
		$settings['about'] = ' ';
		$settings['address'] = $companydata['addressline1'].' '.$companydata['addressline2'];
		$settings['phone'] = $companydata['company_phone'];
		$settings['fax'] = ' ';
		$settings['email'] = $companydata['company_email'];
		$settings['footer'] = ' ';
		$settings['background'] = '';
		...
		$pagesettings['contact_email'] = $companydata['company_email'];
	   ...
	   for($i=0;$i<count($menusname);$i++)
		...
		$message = 'Hello '.$companydata['username'].'  Your Password is: '.$password.'';
	...
	public function temp_add_store(Request $request)
    {
		...
		if($lastid){
			...
			'name' => $company['comapany_name'],
			'description' => $company['comapany_name']. 'Store Registerd on EstoreWhiz',
			'image' => '../assets/superadmin/images/logo.png',
