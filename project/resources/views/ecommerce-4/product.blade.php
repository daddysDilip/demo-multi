        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-md-7 col-sm-7 col-xs-12">
                    ...
                    <div class="detail_box">
                        ...
                        <div class="instock">{{trans('app.Availability')}} : 
                            @if($productdata->stock != 0 || $productdata->stock === null )
                                <span>{{trans('app.InStock')}}</strong>
                            @else
                            ...
                            <div class="signle_addcart_{{$productdata->id}}">
                                @if($productdata->stock != 0 || $productdata->stock === null )
                                    @if((\App\Cart::where('product',$productdata->id)->where('uniqueid', Session::get('uniqueid'))->count()) > 0)
                                        <button class="btn btn-primary"><a href="{{Url('cart')}}">{{trans('app.GoCart')}}</a></button>
                                    @else
                                        <a href="javascript:void(0);" class="btn btn-primary addtocart to-cart">{{trans('app.AddCart')}}<img src="{{url('/assets/images')}}/default.gif" class="to_cart_loader" style="display: block;"></a>
                                    @endif
                                @else
                                    <button class="btn btn-primary out_stock_btn">{{trans('app.OutStock')}}</button>
                                @endif
                            </div>
                        </form>
            ...
            <div id="description" class="row descript-pane">
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#1" data-toggle="tab">{{ trans('app.Description') }}</a>
                    </li>
                    <li>
                        <a href="#2" data-toggle="tab">{{ trans('app.ReturnPolicy') }}</a>
                    </li>
                    <li>
                        <a href="#3" data-toggle="tab">{{ trans('app.Reviews') }}({{\App\Review::reviewCount($productdata->id)}})</a>
                    </li>
                </ul>
    ...
    <div class="container" id="related_product">
      <div id="product-tabs-slider" class="scroll-tabs outer-top-vs wow fadeInUp">
        <div class="more-info-tab clearfix ">
          <h3 class="new-product-title">{{ trans('app.RelatedProducts') }}</h3>
        </div>
                          ...
                          <div class="related_addcart_{{$product->id}}">
                            <li class="add-cart-button btn-group">
                              @if($product->stock != 0 || $product->stock === null )
                                ...
                                @else
                                <button data-toggle="tooltip" class="btn btn-primary icon addTo-cart to-cart" type="button" title="{{$language->add_to_cart}}"><i class="fa fa-shopping-cart"></i><img src="{{url('/assets/images')}}/default.gif" class="to_cart_loader"> {{ trans('app.AddCart') }}</button>
                                @endif
                              @else
