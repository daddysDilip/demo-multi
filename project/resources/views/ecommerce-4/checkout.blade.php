...
<main>
                        ...
                        <div class="login_box">
                            ...
                            <form action="{{ route('user.login.submit',$subdomain_name) }}" method="POST" id="customer_login">
                                ...
                                <div class="form-row">
                                    ...
                                    <input type="submit" class="btn btn-info" value="{{trans('app.Login')}}">
                                </div>
                            </form>
                        </div>
                    </div>
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{trans('app.CouponCode')}}</h5>
    ...
    @endif
    <section id="checkout" class="my-cart-wrap">
        <div class="container">
            <div class="row wrapper">
                
                <div class="col-sm-12">
                  <h2>{{trans('app.Checkout')}}</h2>
                  <hr>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <td>{{trans('app.ProductName')}}</td>
                                    <td>{{trans('app.Quantity')}}</td>
                                    <td>{{trans('app.UnitPrice')}}</td>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td data-title="Quantity" class="quantity">{{$cart->quantity}}</td>
                                    <td data-title="Price" id="price{{$cart->product}}">
                                        {{$settings[0]->currency_sign}}{{number_format($cart->cost,2)}}
                                        <ul>
                                            @if($settings[0]->shipping_information == "Per Product")
                                    ...
                                    <td data-title="Total" class="gtotal">
                                        {{$settings[0]->currency_sign}}{{number_format($display_subtotal,2)}}
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div class="row">
                        <div class="col-sm-8">
                            <div class="form-group update">
                                <input type="text" name="coupon" class="form-control coupon" placeholder="{{trans('app.EnterCouponCode')}}">
                                <button type="submit" class="coupon btn btn-primary apply_code">{{trans('app.Apply')}}</button>
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                <div class="billing-details-area">
                                    ...
                                    <div id="stripes" style="display: none;">
                                        ...
                                        <div class="form-group">
                                            <input type="text" class="form-control" name="month" placeholder="Mo{{trans('app.Month')}}" onkeypress="return isNumber(event)" maxlength="2" minlength="2">
                                        </div>
                                        <div class="form-group">
                    ...
                    else
                    {
                        ...
                        if(response.voucherdetail[0].amounttype == 2)
                        {
                           ...
                           // alert(finalstotal+'_'+ptotaltax+'_'+dtotal);
                            ...
                            $('.discount_val').html(currency+'-'+response.voucherdetail[0].discount.toFixed(2).replace(/\B(?=(?:\d{3})+(?!\d))/g, ','));
                            $('.ptotaltax').html(ptotaltax.toFixed(2).replace(/\B(?=(?:\d{3})+(?!\d))/g, ','));
                            $('.grand_total_val').html(dtotal.toFixed(2).replace(/\B(?=(?:\d{3})+(?!\d))/g, ','));
                        ...
                        else
                        {
                            ...
                            if(maxprice <= mult)
                            {
                               // alert('000');
                                ...
                                $('.discount_val1').html(currency+'-'+maxprice.toFixed(2).replace(/\B(?=(?:\d{3})+(?!\d))/g, ','));
                                $('.grand_total_val').html(dtotal.toFixed(2).replace(/\B(?=(?:\d{3})+(?!\d))/g, ','));
                                $('.ptotaltax').html(ptotaltax.toFixed(2).replace(/\B(?=(?:\d{3})+(?!\d))/g, ','));
                                $('input[name="discountid"]').val(response.voucherdetail[0].id);
                                $('input[name="total"]').val(dtotal);
