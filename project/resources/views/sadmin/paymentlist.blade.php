            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="prtm-block-title mrgn-b-lg">
                        <div class="caption">
                            <a href="{!! url('sadmin/payment/create') !!}" class="btn btn-primary btn-add"><i class="fa fa-plus"></i> Add New Payment</a>
                        </div>
                    </div>
                    <div class="prtm-block-content">
                        <div class="dataTables_wrapper">
                          <!--   <table class="table table-striped table-bordered" cellspacing="0" id="example" width="100%">
                                ...
                                <tbody>
                                    @foreach($payment as $alldata)
                                        <tr>
                                            <td>
                                                @if($alldata->image != '')
                                               <img style="width: 300px;height: 100px;" src="{{url('/')}}/assets/images/payment/{{$alldata->image}}" >
                                                @else 
                                                <img class="service-icon"  src="{{url('/')}}/assets/images/placeholder.jpg" width="120px" height="70px"> 
                                                @endif
                                            </td>
                                            <td>{{$alldata->paymentname}}</td>
                                            <td>{{$alldata->paymenttype}}</td>
                                            <td>
                                                @if($alldata->status == 1)
                                                    <a href="{!! url('sadmin/payment') !!}/status/{{$alldata->id}}/0" class="btn btn-success btn-xs">Active</a>
                                                @elseif($alldata->status == 0)
                                                    <a href="{!! url('sadmin/payment') !!}/status/{{$alldata->id}}/1" class="btn btn-danger btn-xs">Deactive</a>
                                                @endif
                                            </td>
                                            <td>
...
<script type="text/javascript">
    ...
    function delete_data(reportid)
    {
        ...
        {
            window.location = "{{url('/')}}/sadmin/payment/delete/"+reportid;
            return true;
        }
        else
        {
