                ...
                <div class="col-md-9 detail_box"><div class="dashboard-content">
                    ...
                    <div class="dashboard-content">
                        <div class="view-order-page">
                            ...
                            <p class="order-date">{{trans('app.OrderDate')}}: {{$order->booking_date}}</p><br>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered veiw-details-table">
                                    ...
                                    <tr style="font-weight: 600;">
                                        <td colspan="3" class="pr_total">{{trans('app.SubTotal')}}</td>
                                    ...
                                    <tr style="font-weight: 600;">
                                        <td colspan="3" class="pr_total">{{trans('app.Discount')}}<br>({{$order->discount_code}})</td>
                                    ...
                                    @if($settings[0]->shipping_information  == 'Per Order')
                                        ...
                                        <tr style="font-weight: 600;">
                                            <td colspan="3" class="pr_total">{{trans('app.ShippingCost')}}</td>
                                    ...
                                    @if($settings[0]->tax_information  == 'Per Order')
                                        @if($order->tax != 0)
                                        <tr style="font-weight: 600;">
                                            <td colspan="3" class="pr_total">{{trans('app.Tax')}}({{$order->tax}}%)</td>
                                            @if(!empty($order->discount_code))
                                                @php 
                                                    $tax = $order->tax;
                                            ...
                                            @else
                                                ...
                                                @endphp
                                            @endif
                                            <td align="right" class="pr_total">{{$settings[0]->currency_sign}}{{number_format($totaltax,2)}}</td>
                                        </tr>
                                        @endif
                                    @endif
                                    <tr style="font-weight: 600;">
                                        <td colspan="3" class="pr_total">{{trans('app.Total')}}</td>
                            ...
                            <div class="edit-account-info-div">
                                <div class="form-group">
                                    <a class="btn btn-md view_order back-btn" href="{{route('user.orders',$subdomain_name)}}">{{trans('app.Back')}}</a>
                                    <a class="btn btn-md back-btn pull-right" href="{{url('user/orderpdf/')}}/{{$order->id}}">{{trans('app.DownloadInvoice')}}</a>
                                </div>
                            </div>
