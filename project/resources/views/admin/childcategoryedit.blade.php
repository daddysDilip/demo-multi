        ...
        <div class="prtm-content">
            <div class="prtm-page-bar">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">Chlid Category</li>
                </ul>
            </div>
                <!-- Page Content -->
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <div id="response"></div>
                        <div class="col-xs-12">
                            <form method="POST" action="{{url('admin/childcategory')}}/{{$category->id}}" class="form-horizontal form-label-left" enctype="multipart/form-data" id="category_form">
                                {{csrf_field()}}
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane active" id="default_lang">
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <select class="form-control" name="mainid" id="maincats" required>
                                                    ...
                                                    @foreach($categories as $cat)
                                                        @if(\App\CategoryTranslations::where('categoryid',$cat->id)->where('langcode',get_defaultlanguage() )->count() > 0)
                                                            <option value="{{$cat->id}}" @if($cat->id == $category->mainid->id) selected @endif>{{ \App\CategoryTranslations::where('categoryid',$cat->id)->where('langcode',get_defaultlanguage() )->first()->name }}</option>
                                                        @endif    
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <select class="form-control" name="subid" id="subs" required>
                                                    ...
                                                    @foreach($subcats as $subcat)
                                                        @if(\App\CategoryTranslations::where('categoryid',$subcat->id)->where('langcode',get_defaultlanguage() )->count() > 0)
                                                            <option value="{{$subcat->id}}" @if($subcat->id == $category->subid->id) selected @endif>{{ \App\CategoryTranslations::where('categoryid',$subcat->id)->where('langcode',get_defaultlanguage() )->first()->name }}</option>
                                                        @endif    
                                                    @endforeach
                                                </select>
                                            </div>
                                        ...
                                        <div class="item form-group">
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <input id="name" class="form-control col-md-7 col-xs-12" name="name" value="{{$category->name}}" placeholder="e.g Sports" type="text" maxlength="30" minlength="3">
                                            </div>
                                        </div>
                                        <div class="item form-group">
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <input id="slug" class="form-control col-md-7 col-xs-12" name="slug" value="{{$category->slug}}" placeholder="e.g sports" type="text" maxlength="30" minlength="3">
                                            </div>
                                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            url: "{{ URL('admin/delete/chiledcategory') }}/"+id,
            type: "get",
            async: false,
