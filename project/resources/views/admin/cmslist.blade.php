        ...
        <div class="prtm-content">
            <div class="prtm-page-bar">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">{{trans('app.PageSection')}}</li>
                </ul>
            </div>
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="prtm-block-title mrgn-b-lg">
                        <div class="caption">
                            <a href="{!! url('admin/cms/create') !!}" class="btn btn-primary btn-add"><i class="fa fa-plus"></i> {{trans('app.AddNew')}}</a>
                            @if(count($cms) > 0)
                                <a href="{!! url('admin/exportcms') !!}" class="btn btn-primary btn-add"><i class="fa fa-download"></i> {{trans('app.Export')}}</a>
                            @endif
                            <button type="button" class="btn btn-primary btn-add" data-toggle="modal" data-target="#modal-default"><i class="fa fa-file-excel-o"></i> {{trans('app.Import')}}</button>
                             ...
                             <a href="http://adminuser.multiecom.com/assets/importtile/CMS.xls"><button type="button" class="btn btn-primary btn-add"><i class="fa fa-download"></i> Demo File</button>
                        </div>
                    </div>
                     ...
                     <table class="table table-striped table-bordered" cellspacing="0" id="posts" width="100%">
                        <thead>
                            <tr class="bg-primary">
                                <th>{{trans('app.PageName')}}</th>
                                <th>{{trans('app.PageTitle')}}</th>
                                <th>{{trans('app.PublishDate')}}</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{!! action('CmsController@import',$subdomain_name) !!}" class="form-horizontal form-label-left" enctype="multipart/form-data" id="blog_form">
                        {{csrf_field()}}
...
$(document).ready(function () {
    $('#posts').DataTable({
        ...
        "serverSide": true,
        "ajax":{
                 "url": "{{ url('admin/allpostsadmincms') }}",
                 "dataType": "json",
                 "type": "POST",
...
function delete_data(reportid)
{
    ...
    {
        window.location = "{{url('/')}}/admin/cms/delete/"+reportid;
        return true;
    }
    else
    {
