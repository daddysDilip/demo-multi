        ...
        <div class="prtm-content">
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="col-xs-12">
                        <form method="POST" action="{{url('admin/subcategory')}}/{{$category->id}}" class="form-horizontal form-label-left" enctype="multipart/form-data" id="category_form">
                            {{csrf_field()}}
                                            ...
                                            <select class="form-control" name="mainid">
                                                ...
                                                @foreach($categories as $cat)
                                                    @if(\App\CategoryTranslations::where('categoryid',$category->id)->where('langcode',get_defaultlanguage() )->count() > 0)
                                                        <option value="{{$cat->id}}" @if($cat->id == $category->mainid->id) selected @endif>{{ \App\CategoryTranslations::where('categoryid',$category->id)->where('langcode',get_defaultlanguage() )->first()->name }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        </div>
...
<script type="text/javascript">
    ...
    $('.delete_img').click(function(){
        $.ajax({
            url: "{{ URL('admin/delete/subcategory') }}/"+id,
            type: "get",
            async: false,
