            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="prtm-block-title mrgn-b-lg">
                        <div class="caption">
                            <a href="{!! url('admin/user/create') !!}" class="btn btn-primary btn-add"><i class="fa fa-plus"></i> Add New User</a>
                        </div>
                    </div>
                        ...
                        @foreach($user as $alluser)
                            <tr>
                                ...
                                @foreach($sadmin as $alldata)
                                    @php $sid[]= $alldata->id; @endphp
                                @endforeach
                                
                                <td>
                                    @if(!in_array($alluser->created_by, $sid))
                                        @if($alluser->id != $userid)
                                            @if($alluser->status == 1)
                                                <a href="{!! url('admin/user') !!}/status/{{$alluser->id}}/0" class="btn btn-success btn-xs">Active</a>
                                            @elseif($alluser->status == 0)
                                                <a href="{!! url('admin/user') !!}/status/{{$alluser->id}}/1" class="btn btn-danger btn-xs">Deactive</a>
                                            @endif
                                        @endif
                                    @endif
                                </td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('#posts').DataTable({
            ...
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('admin/allpostsadminuser') }}",
                     "dataType": "json",
                     "type": "POST",
...
<script type="text/javascript">
    ...
    function delete_data(reportid)
    {
        ...
        {
            window.location = "{{url('/')}}/admin/user/delete/"+reportid;
            return true;
        }
        else
        {
